...
window.onload = function () {
	...
	for (var _i = 0; _i < demoList.length; _i++) {
		demoList[_i].onclick = demoChange();
	};
...
function demoChange() {
	return function () {
		...
		for (var i = 0; i < demo.length; i++) {
			if (className == "All") $(demo[i]).show();else {
				if ($(demo[i]).hasClass(className)) $(demo[i]).show();else {
					$(demo[i]).hide();
				}
			}
		}
...
function isTop() {
	return function () {
		...
		var toTop = document.getElementById("toTop");
		if (top > 100) toTop.style.bottom = "5%";else toTop.style.bottom = "-5%";
	};
}